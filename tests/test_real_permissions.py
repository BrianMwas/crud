"""
Test real-world permission scenarios for the resource system.

This test implements the following scenarios with actual database interactions:
...
from app.models import Resource, Base
...
def test_real_permissions():
    """
    Test the resource permission system with real database interactions.
    ...
    """
    print("\n" + "=" * 80)
    print("TESTING REAL RESOURCE PERMISSIONS")
    print("=" * 80)
    ...
    print("\n5. Testing M2M access to a different model class (Document)...")

    # Create a document (Object C)
    headers = {"Authorization": f"Bearer {user_token}"}
    document_data = {"title": "Document C", "content": "This is a document for testing permissions"}

    # Create the document
    response = requests.post(f"{BASE_URL}/documents/", json=document_data, headers=headers)

    if response.status_code == 201:
        document_c = response.json()
        print(f"✅ Created Document C with ID {document_c['id']}")

        # Now try to access the document with the M2M token
        # This should fail because the M2M token doesn't have 'read:documents' permission
        headers = {"Authorization": f"Bearer {m2m_token if is_m2m_m2m else user_token}"}
        response = requests.get(f"{BASE_URL}/documents/{document_c['id']}", headers=headers)
        ...
        print(f"❌ Failed to create Document C: {response.status_code} - {response.text}")
        print("   Skipping Document C access test")
